...
- ✅ **实验管理**：基于分组的实验组织，支持树形结构
- ✅ **效果看板**：多实验对比，支持基线选择
- ✅ **对比指标**：详细的评估器分数和运行时指标对比
- ✅ **数据导出**：导出对比结果用于进一步分析

### v1.1.1

**实验 Prompt 评测** - 新增实验页面支持 Prompt 评测功能：

- ✅ **Prompt 作为评测对象**：支持在实验中选择 Prompt 作为评测对象
- ✅ **用户输入映射**：必填的用户输入字段映射到数据集字段
- ✅ **变量映射**：可选的 Prompt 变量映射到数据集字段
- ✅ **默认变量值**：未配置映射时使用 Prompt 管理中配置的变量值
- ✅ **版本支持**：支持选择 Prompt 版本或使用草稿进行评测
